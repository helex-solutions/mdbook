// Neutralizes Owliki `{{ … }}` embed directives (and any other double-brace
...
// `owliki-embed` class, pending real build-time expansion.
...
function includeBlock(md) {
  ...
  md.renderer.rules.owliki_include = (tokens, idx) => {
...
export function owlikiEmbeds(md) {
  includeBlock(md)

  md.inline.ruler.before('escape', 'owliki_braces', (state, silent) => {
    const src = state.src
    ...
    if (!silent) {
      ...
      const token = state.push('owliki_embed', 'code', 0)
  ...
  md.renderer.rules.owliki_embed = (tokens, idx) => {
    const t = tokens[idx]
    const cls = t.meta?.embed ? ' class="owliki-embed"' : ''
