...
// Build a minimal Owliki project: __source/{space,pages}.json + input/<slug>.md.
function tmpOwliki(space, pages, pageBodies = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'mdbook-owliki-'))
...
const cfgFor = (dir, lang = 'en') => ({
  projectRoot: dir,
  source: { format: 'owliki', meta: '__source', pages: 'input' },
  site: { lang, title: null, web: null }
})

test('owliki ingest: reads space defaultLang/langs/description/siteUrl + page description/keywords', () => {
  const space = {
    code: 'sp1',
    ...
    ssg: { theme: { skin: 'helex' }, footer: { message: 'Guide' }, txServer: 'https://tx.example.org/api/fhir' }
  }
  const pages = [
    {
      code: 'p1',
  ...
  const dir = tmpOwliki(space, pages, { home: '# Home\n\nBody.', pradzia: '# Pradžia\n\nTekstas.' })
  const model = ingestOwliki(cfgFor(dir, 'en'))
  ...
  assert.equal(model.ssg?.txServer, 'https://tx.example.org/api/fhir')
...
test('owliki ingest: falls back cleanly when the new fields are absent', () => {
...
test('owliki ingest: locale-switch redirect stubs for slugs that differ per language', () => {
  ...
  const dir = tmpOwliki(space, pages, { build: '# Builds\n', versijos: '# Versijos\n' })
  const model = ingestOwliki(cfgFor(dir, 'en'))
